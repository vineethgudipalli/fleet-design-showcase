...
export default function FullscreenPrototypeModal({
    ...
    // Use embedUrl from prototype data, fallback to demo URL
    const embedUrl = prototype.embedUrl || `https://www.figma.com/embed?embed_host=share&url=https://www.figma.com/proto/demo/${prototype.id}`;

    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="!max-w-none !max-h-none !w-screen !h-screen !p-0 !gap-0 !bg-[#1e1e1e] !fixed !inset-0 !top-0 !left-0 !transform-none !translate-x-0 !translate-y-0 !rounded-none !border-0 [&>button]:hidden">
                <DialogTitle className="sr-only">{prototype.title} - Prototype Viewer</DialogTitle>
                ...
                <div className="flex h-screen w-full">
                    ...
                    <div className="flex-[3] flex flex-col bg-[#1e1e1e] overflow-hidden">
                        {/* Prototype iframe */}
                    ...
                    <div className="flex-[1] border-l border-[#3e3e3e] bg-[#2c2c2c] min-w-[300px] overflow-hidden relative">
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {prototype.tags.map(tag => (
                                        <Badge key={tag} variant="secondary" className="bg-white/10 text-white/70 hover:bg-white/20">
                                            {tag}
                                        </Badge>
                                    ))}
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <Avatar className="w-10 h-10">
                                        ...
                                        <AvatarFallback className="bg-[#0d99ff] text-white">{prototype.author.initials}</AvatarFallback>
                                    </Avatar>
                                    <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {prototype.persona.map(persona => (
                                        <Badge key={persona} className="bg-[#0d99ff]/20 text-[#0d99ff] hover:bg-[#0d99ff]/30">
                                            {persona}
                                        </Badge>
                                    ))}
                                </div>
                                ...
                                <div>
                                    <PrototypeReactions
                                        ...
                                        onReact={onReactionToggle ? handleReactionToggle : undefined}
                                        size="lg"
                                    />
                                </div>
                                ...
                                {/* Comments Section */}
                                <div>
                                    <h4 className="text-white font-medium mb-3">Comments ({prototype.comments.length})</h4>
                                    ...
                                    {isAuthenticated ? (
                                        ...
                                        <div className="p-4 bg-[#3e3e3e] rounded-lg text-center text-white/60 mb-6">Sign in to comment</div>
                                    )}
